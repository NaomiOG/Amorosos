	    				...
	    				buttons: {
        						confirmar:{
        						 ...
        						 action: function () {
            							...
            							type:"POST",
            							method:"POST",
            							url: "home.php?id="+id+"&accion=genero.delete", 
  										});
  										$(location).attr('href',"home.php?id="+id+"&accion=genero.delete");
...
if(!class_exists("baseDatos"))
	...
	class Genero extends baseDatos{
		...
		function accion($cual,$id=-1){
			...
			switch ($cual) {
				...
				case 'list':
				   ...
				   $result=$this->showTabla("select id,tipo as Tipo from genero",array("5%","90%"),true,array("edit","delete"));

				break;

				case 'insert':
			 
				$this->consulta("insert into genero set tipo='".$_POST['tipo']."'");
				
				$result=$this->accion("list");
				//$result=$this->accion("carac");
				break;
				case 'update':
		        $this->consulta("UPDATE genero set tipo='".$_POST['tipo']."' where id=".$_POST['id']);
		        $result=$this->accion("list"); 
		    	break;	

				case 'delete':
					$this->consulta("delete from genero where id=".$id);
				...
				case "formEdit":  $row=$this->saca_tupla("SELECT * from genero where id=".$_REQUEST['id']);
				...
				case 'formNew':
					...
					<form method="post">
						<div class="form-group">
	     				<label>Tipo</label>
	      				<input type="text" class="form-control" id="tipo" name ="tipo" placeholder="Ingresa el tipo" value="'.((isset($row->tipo))?$row->tipo:'').'">
	    				</div>';

					$result.=(isset($row->tipo)?'<button class="btn btn-sm- btn-primary">Actualizar</button><input type="hidden" name="id" value="'.$row->id.'"/><input type="hidden" name="accion" value="genero.update"     />':'<button class="btn btn-sm- btn-primary">Registrar</button><input type="hidden" name="accion" value="genero.insert" />');
		...
		<body>';
		$result.='<div class="container" style="margin-top: 20px;"><span class="badge badge-pill badge-primary">Género</span><table class="table">';
		#cabecera de las columnas

		$result.='<tr class=table-success>';
		$result.='<td colspan="'.count($p_iconos).'">'.(($agregar)?'<a href="?accion=genero.new"><img id="add" src="../imagenes/img_trans.png"/></a>':"&nbsp;").'</td>';
		...
		for ($contR=0; $contR < $this->numTuplas; $contR++) { 
			...
			if(in_array("edit", $p_iconos)){
				$result.='<td width="5%"><a href="?id='.$tupla[0].'&accion=genero.formEdit"><img id="edit" src="../imagenes/img_trans.png"/></td>';
			}
